...
class MatchService {
  ...
  Future<List<MatchCard>> getPendingMatches() async {
    ...
    final rows = await supabase
        .from('matches')
        .select('*, events(event_name), user1:user1_id(id, name, university, course, bio, year_group, user_interests(interest)), user2:user2_id(id, name, university, course, bio, year_group, user_interests(interest))')
    ...
    for (final row in rows as List) {
      ...
      final eventName = (row['events'] as Map<String, dynamic>?)?['event_name'] ?? 'Unknown Event';

      matches.add(MatchCard(
  ...
  Future<void> recordDecision(String matchId, bool accepted) async {
    ...
    await supabase
        .from('matches')
        .update({
          if (isUser1) 'user1_accepted': accepted else 'user2_accepted': accepted,
        })
        .eq('user1_id', user1Id)
        .eq('user2_id', user2Id)
        .eq('event_id', eventId);
  ...
  Future<List<ChatConversation>> getConversations() async {
    final rows = await supabase
        .from('matches')
        .select('*, user1:user1_id(id, name, user_interests(interest)), user2:user2_id(id, name, user_interests(interest))')
